		...
		<view class="inner-wrapper">
			...
			<view class="navs">
				<view class="c-flex-align" :class="{ 'c-flex-center': index > 0, actNav: index === actNav }"
					...
					<view v-for="(child, childx) in item" :key="childx" v-if="child.select" style="color: #333333;">{{ child.text }}</view>
					<image src="../../static/img/home/shang.png" class="icon-triangle"
						v-if="index === actNav"></image>
					<image src="../../static/img/home/xia.png" class="icon-triangle" v-else>
					</image>
				</view>
	...
	export default {
		...
		methods: {
			...
			handleOpt(index) {
				...
				this.$emit('click', {
					index1:this.actNav,
					index: this.navData[this.actNav][index].value
				});
				// console.log(data);
